...
function renderMain() {
  ...
  if (bgPal) {
    ...
    const subtleC = bgPal.shades[Math.floor(bgPal.shades.length * 0.65)]?.hex || '#666';
    const txtC2 = txtPal.shades[txtPal.shades.length - 1]?.hex || txtC;
    const txtsC2 = txtsPal.shades[Math.floor(txtsPal.shades.length * 0.55)]?.hex || subtleC;
...
function renderPalCard(p) {
  ...
  return `<div class="pal-card">
    ...
    <div class="shades-wrap">
      ${p.shades
        .map((s) => {
          const lc1 = apca(s.hex, ctBg1),
            lc2 = apca(s.hex, ctBg2);
          const badge = (lc, bg) => {
            const a = Math.abs(lc);
            const cls = a >= 75 ? 'ct-aaa' : a >= 60 ? 'ct-aa' : 'ct-fail';
            const lbl = a >= 45 ? 'Lc' + Math.round(a) : 'Lc' + Math.round(a);
            return `<div class="ct-row ${cls}"><div class="ct-dot" style="background:${bg}"></div>${lbl}</div>`;
          };
...
function applyCPColor(hex) {
  ...
  if (activeCPId === 'bg1') {
    ctBg1 = hex;
    document.getElementById('btn-bg1').style.background = hex;
    rebuild();
    persistState();
  } else if (activeCPId === 'bg2') {
    ctBg2 = hex;
    document.getElementById('btn-bg2').style.background = hex;
    rebuild();
    persistState();
  } else {
